...
namespace FiguresDrawer.View.Forms
{
	public partial class Index : Form, IFiguresDrawerView
	{
		...
		public Index()
		{
			...
			// Maybe do some refactoring here
			//canvas.MouseMove	+= (s, args) => { Mouse_Moving.Invoke(s, args); };
			//canvas.MouseUp	+= (s, args) => { Mouse_KeyUp.Invoke(s, args); };
			//canvas.MouseDown	+= (s, args) => { Mouse_KeyDown.Invoke(s, args); };
		}
